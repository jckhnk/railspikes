class Test2SpikesKoi < ActiveRecord::Base
  ...
  def to_csv_xy(x, y)
  	return Test2SpikesKoi.csv_column_names.map{|c| self.send(c)} + [self.send(x), self.send(y)]
  end

  def to_csv
  	return Test2SpikesKoi.column_names.map{|c| self.send(c)}
  end
  ...
  def build_plot_data(x, y)
    ...
    data << self.send(x)
    data << self.send(y)
    Test2SpikesKoi.column_names.each do |c|
