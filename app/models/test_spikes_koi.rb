class TestSpikesKoi < ActiveRecord::Base
  ...
  def to_csv_xy(x, y)
  	return TestSpikesKoi.csv_column_names.map{|c| self.send(c)} + [self.send(x), self.send(y)]
  end

  def to_csv
  	return TestSpikesKoi.column_names.map{|c| self.send(c)}
  end
  ...
  def build_plot_data(x, y)
    ...
    data << self.send(x)
    data << self.send(y)
    TestSpikesKoi.column_names.each do |c|
